//
//  KasaiTextField.swift
//  TextFieldEffects
//
//  Created by Mushthak Ebrahim on 09/08/17.
...
/**
 An KasaiTextField is a subclass of the TextFieldEffects object, is a control that displays an UITextField with a customizable visual effect around the edges and background of the control.
 */
@IBDesignable open class KasaiTextField: TextFieldEffects {
    
    private let borderLayer = CALayer()
    private let textFieldInsets = CGPoint(x: 15, y: 0)
    private let placeHolderInsets = CGPoint(x: 13, y: 0)
    ...
    /**
     ...
     This property applies a color to the complete placeholder string. The default value for this property is a dark gray color.
     */
    @IBInspectable dynamic open var activePlaceholderColor: UIColor = .darkGray {
        didSet {
            updatePlaceholder()
        }
    }
     ...
     */
    @IBInspectable dynamic open var placeholderFontScale: CGFloat = 0.6 {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    /**
     The corner radius of text field
     
     This property determines the corner radius of the text field.
     */
    @IBInspectable dynamic open var cornerRadius: CGFloat = 5 {
        didSet {
            updateBorder()
        }
    }
    ...
    private func updatePlaceholder() {
        ...
        if isFirstResponder || text!.isNotEmpty {
            ...
            placeholderLabel.font = UIFont.boldSystemFont(ofSize: placeholderLabel.font.pointSize)
    ...
    open override func placeholderRect(forBounds bounds: CGRect) -> CGRect {
        if isFirstResponder || text!.isNotEmpty {
            return CGRect(x: placeHolderInsets.x, y: placeHolderInsets.y + 20, width: bounds.width, height: placeholderHeight)
        } else {
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        if isFirstResponder || text!.isNotEmpty {
            return bounds.offsetBy(dx: textFieldInsets.x, dy: textFieldInsets.y + 5 + placeholderHeight / 2)
        }
